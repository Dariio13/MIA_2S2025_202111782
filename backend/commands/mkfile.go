...
func commandMkfile(mkfile *MKFILE) error {
    ...
    // 🔹 OBTENER PARTICIÓN ACTUAL DESDE LA SESIÓN
    _, _, partitionID := stores.Auth.GetCurrentUser()
    fmt.Println("Sesión activa en partición:", partitionID)

    // Obtener la partición montada y su superbloque
    partitionSuperblock, mountedPartition, partitionPath, err := stores.GetMountedPartitionSuperblock(partitionID)
    if err != nil {
        return fmt.Errorf("error al obtener la partición montada: %w", err)
    }

    // Si no hay contenido, generar datos aleatorios para llenar el tamaño
    if mkfile.cont == "" {
