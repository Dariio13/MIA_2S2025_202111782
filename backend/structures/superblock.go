...
func (sb *SuperBlock) ReadFile(path string, filePath string) ([]byte, error) {
// Buscar el bloque de usuarios (users.txt)
block, err := sb.GetUsersBlock(path)
if err != nil {
return nil, err
}

// Retornar el contenido del archivo (el bloque de datos)
return block.B_content[:], nil
}

// UpdateFile guarda el contenido en el archivo especificado dentro del sistema de archivos
func (sb *SuperBlock) UpdateFile(path string, filePath string, content []byte) error {
...
err = block.Serialize(path, int64(sb.S_block_start+(block.InodeID*sb.S_block_size)))
if err != nil {
return err
}
